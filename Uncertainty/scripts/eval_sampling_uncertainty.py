"""run multiple distribution and save the result. For the pure depth version."""
...
def eval_model(
    model, dataloader, save_name: str, n_sample: int, centers=(14,), widths=(0,), batch_size=16, downsample=5
):
    ...
    final_result = {'abs_error': abs_error, 'sampled_uncertainty': sampled_uncertainty}
...
def analyze_result(n_sample: int):
    ...
    print(f"Analyzing results for Ours-{n_sample}")
    result_paths = Path("./temp").glob(f"depth_*_N{n_sample}.pth")
    error_uctt = merge_uncertainty_files(sorted(result_paths))
    l1_err_result, uq_std_result, uq_mean_result, uq_mix_result = analyze_uncertainty(error_uctt, utype='depth')

    # print results
    nll = uq_std_result[(14, 14)]["nll_laplace"]
    ause = uq_std_result[(14, 14)]["ause"] * 10
    print(f"  Ours-{n_sample}: NLL = {nll:.3f}, AUSE = {ause:.3f}")


def main():
    log_paths = sorted(Path("./logs").glob("depth_*"))
